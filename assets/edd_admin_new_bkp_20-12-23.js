...
function checkGoogleBoatAgent(nAgt){
  const robots = new RegExp([
   ...
   // console.log(typeof check_edd_data_from_liquid !== 'undefined')
    ...
    if(!checKGoogleBoat && typeof check_edd_data_from_liquid !== 'undefined' &&  check_edd_data_from_liquid == '1'){
    ...
    for(var aeddsb=0;aeddsb<shortcodeList.length;aeddsb++){
     // console.log('');
      ...
      if(is_numeric && (checkSHortcode.indexOf('min') != -1 || checkSHortcode.indexOf('max') != -1)){
        ...
        if(check_cutoff == 'less' && check_currentDay_is_weekoff == 0 ){
          getValueFromShortcode = getValueFromShortcode+1;
         // console.log('=> check cut off & new value is '+getValueFromShortcode);
        }
        ...
        for(keddsb =0 ; keddsb<=getValueFromShortcode; keddsb++){
          getNextDay = keddsb+1;
          var user_current_day = weekday[date.getDay()];
          //console.log(date+' => user_current_day '+user_current_day+' value '+getValueFromShortcode);

          /* check for Wednesday =>  after cutofff time */
          // if(check_currentDay_is_weekoff == 0 && check_nextDay_is_weekoff == 0 && check_next2Day_is_weekoff == 1 && check_next2Day_is_weekoff == 1){
          //  currentDate = date.setDate(date.getDate());
          //   console.log('currentDate '+ new Date(currentDate))
          //   if(checkOffDay($, currentDate) ){
          //     console.log('next day is off day')
          //     //getValueFromShortcode = getValueFromShortcode + 1;
          //   }
          // }
          
           /* check for thursday => before & after cutofff time */
          if(check_currentDay_is_weekoff == 0 && check_nextDay_is_weekoff == 0 && check_next1Day_is_weekoff == 1){
           // console.log('********* thursday ************8')
           currentDate = date.setDate(date.getDate());
            console.log('currentDate '+ new Date(currentDate))
            if(checkOffDay($, currentDate) ){
             // console.log('next day is off day')
          ...
          /* check for friday => before cutofff time */
          if(add_1_day_if_current_working_next_is_off == 0 && check_currentDay_is_weekoff == 0 && check_nextDay_is_weekoff == 1 && check_cutoff != 'less'  && check_nextDay_is_weekoff == 1  && checkOffDay($, date)){
           // console.log('current day is wroking day & next day is offday & before cutoff time')
            add_1_day_if_current_working_next_is_off = 1;
            getValueFromShortcode = getValueFromShortcode + 1;
            
            nextDate = new Date(date.setDate(date.getDate() + 1));
            var user_next_day = weekday[nextDate.getDay()];
            if(checkOffDay($, nextDate) && getValueFromShortcode == 2){
              //console.log('next day is off day')
              getValueFromShortcode = getValueFromShortcode + 1;
              add_1_day_if_current_working_next_is_off = 1;
            }
          ...
          /* check for friday => after cutofff time */
          if(check_currentDay_is_weekoff == 0 && check_nextDay_is_weekoff == 1 && check_cutoff == 'less' && checkOffDay($, date)){
           // console.log('current day is wroking day & next day is offday & after cutoff time')
            getValueFromShortcode = getValueFromShortcode + 1;
          }
          
          /* check for saturday */
          if(add_1_day_if_current_next_is_weekoff == 0 && check_currentDay_is_weekoff == 1 && check_nextDay_is_weekoff == 1 &&  checkOffDay($, date) ){
            //console.log('get week off day & next day is offday')
            getValueFromShortcode = getValueFromShortcode + 1;
            ...
            if(checkOffDay($, nextDate)){
              getValueFromShortcode = getValueFromShortcode + 1;
              add_1_day_if_current_next_is_weekoff = 1;
            }
           // console.log(nextDate)
          }

          /* check for Sunday */
          if(add_1_day_if_current_off_next_is_working == 0 && check_currentDay_is_weekoff == 1 && check_nextDay_is_weekoff == 0 &&  checkOffDay($, date) ){
           // console.log('get week off day  & next day is working')
            getValueFromShortcode = getValueFromShortcode + 1;
